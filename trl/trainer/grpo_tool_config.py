...
@dataclass
class GRPOToolConfig(GRPOConfig):
    r"""
    Configuration class for the [`GRPOToolTrainer`].

    This class includes only the parameters that are specific to GRPO with tool call training. For a full list of training arguments,
    ...
    Parameters:
        ...
        mask_tool_output(`bool`, *optional*, defaults to `True`):
            When enabled, tool call outputs' tokens are excluded from loss calculation. According to the
            [Search-R1](https://huggingface.co/papers/2503.09516) paper, this improves training performance.
    ...
    mask_tool_output: bool = field(
        default=True,
        metadata={
            "help": "When enabled, tool call outputs' tokens are excluded from loss calculation. According to the "
            "Search-R1 paper, this improves training performance."
        },
    )
